defmodule Jump.TimeHelpersUnitTest do
  use ExUnit.Case
  ...
  describe "beginning_of_week/1" do
    test "returns Monday for dates in week" do
  ...
  describe "end_of_week/1" do
    test "returns Sunday for dates in week" do
      monday = DateTime.new!(~D[2024-01-15], ~T[10:00:00], "Etc/UTC")
  ...
  describe "beginning_of_month/1" do
    ...
    test "handles different months" do
      jan = DateTime.new!(~D[2024-01-15], ~T[10:00:00], "Etc/UTC")
      feb = DateTime.new!(~D[2024-02-28], ~T[10:00:00], "Etc/UTC")
      dec = DateTime.new!(~D[2024-12-31], ~T[10:00:00], "Etc/UTC")

      assert DateTime.to_date(TimeHelpers.beginning_of_month(jan)) == ~D[2024-01-01]
      assert DateTime.to_date(TimeHelpers.beginning_of_month(feb)) == ~D[2024-02-01]
      assert DateTime.to_date(TimeHelpers.beginning_of_month(dec)) == ~D[2024-12-01]
    end
  end

  describe "end_of_month/1" do
    test "returns last day of month" do
      dt = DateTime.new!(~D[2024-01-15], ~T[14:30:45], "Etc/UTC")
    ...
    test "handles months with different day counts" do
      jan = DateTime.new!(~D[2024-01-15], ~T[10:00:00], "Etc/UTC")
      feb = DateTime.new!(~D[2024-02-15], ~T[10:00:00], "Etc/UTC")
      apr = DateTime.new!(~D[2024-04-15], ~T[10:00:00], "Etc/UTC")

      assert DateTime.to_date(TimeHelpers.end_of_month(jan)) == ~D[2024-01-31]
      assert DateTime.to_date(TimeHelpers.end_of_month(feb)) == ~D[2024-02-29]
      assert DateTime.to_date(TimeHelpers.end_of_month(apr)) == ~D[2024-04-30]
    end
  end

  describe "beginning_of_year/1" do
    test "returns first day of year" do
  ...
  describe "end_of_year/1" do
    test "returns last day of year" do
      dt = DateTime.new!(~D[2024-06-15], ~T[14:30:45], "Etc/UTC")
